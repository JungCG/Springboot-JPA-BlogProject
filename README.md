...
- **서블릿 컨테이너**
    - 클라이언트가 요청을 한다. 서블릿 컨테이너(톰켓)이 요청을 받고 1) 최초 요청일 경우 메모리 로딩-객체생성-init 2) 최초 요청이 아니면 객체를 생성하지 않고 이미 생성된 객체를 재사용하고 Service(HttpServletRequest, HttpServletResponse)
